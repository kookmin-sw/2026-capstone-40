...
class StealthScraper:
    ...
    def scrape(self) -> dict:
        """Scrape the URL and store results under a timestamped snapshot folder."""
        scraped_at = datetime.now(timezone.utc)
        timestamp = scraped_at.strftime("%Y-%m-%d_%H-%M-%S")

        self._launch_browser()
        try:
            domain_slug = self.domain.replace(".", "_")
            snapshot_dir = self.output_dir / domain_slug / timestamp
            snapshot_dir.mkdir(parents=True, exist_ok=True)

            page_html = self._fetch_page_html()
            html_file_path = snapshot_dir / "index.html"
            html_file_path.write_text(page_html, encoding="utf-8")
            ...
            assets_folder = snapshot_dir / "assets"
            assets_folder.mkdir(exist_ok=True)

            downloaded = 0
            for index, asset_url in enumerate(discovered_assets[: self.config.max_assets]):
                file_extension = Path(urlparse(asset_url).path).suffix or ".bin"
                asset_file_path = assets_folder / f"asset_{index}{file_extension}"
                if self._download_asset(asset_url, asset_file_path):
                    downloaded += 1
