...
pub fn handle(domain: &str, db: &Db) -> response::HttpResponse {
    ...
    let (risk_score, _decision) = store::domain_detail(&conn, domain).unwrap_or((None, None));
    ...
    let ip_history: Vec<templates::IpRecord> = store::domain_ip_history(&conn, domain)
        .into_iter()
        .map(|(ip, first_seen, last_seen)| templates::IpRecord {
            ip,
            first_seen,
            last_seen,
        })
    ...
    let last_failed_probe = store::last_failed_probe_ts(&conn, domain);
