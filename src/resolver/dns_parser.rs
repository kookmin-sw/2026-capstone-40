//! DNS wire-format parser for passive DNS capture.
//!
//! Parses raw Ethernet frames containing UDP port-53 DNS responses.
//! No external crates — pure byte parsing of RFC 1035 wire format.
//!
//! Entry point: `parse_dns_from_frame(data)` → Vec<(IpAddr, domain, ttl)>.

use std::collections::{HashMap, HashSet};
use std::net::{IpAddr, Ipv4Addr, Ipv6Addr};

use super::types::{looks_like_domain, norm_domain};

pub fn parse_dns_from_frame(data: &[u8]) -> Vec<(IpAddr, String, u32)> {
    if data.len() < 14 {
        return vec![];
    }
...
fn parse_dns_records(
    ...
    // useful than the canonical CDN name returned in the answer section.
    for _ in 0..qdcount {
...
mod tests {
    ...
    fn make_dns_response(domain: &str, ip: [u8; 4], ttl: u32) -> Vec<u8> {
        ...
        msg.extend_from_slice(&[0xC0, qname_start as u8]);
